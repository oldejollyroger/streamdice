...
if (filters.ageRatingMin > 0 || filters.ageRatingMax > 0) {
  ...
  const selectedRatings = ageRatingOptions.slice(min, max + 1).filter(r => r !== t.any);
...
with_watch_monetization_types: filters.platform.length > 0 ? 'flatrate' : 'flatrate|free|ads|rent|buy',
...
const handleActorClick = async (actorId) => {
  ...
  console.log('modal should be open now'); // ADD THIS
